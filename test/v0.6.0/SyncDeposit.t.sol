...
contract TestSyncDeposit is BaseTest {
    ...
    function test_requestDeposit_succeedsWhenSyncModeIsNone() public {
        vm.prank(vault.safe());
        vault.setSyncMode(SyncMode.None);
    ...
    function test_requestRedeem_succeedsWhenSyncModeIsNone() public {
        test_syncDeposit_simple();
        uint256 half = vault.balanceOf(user1.addr) / 2;

        vm.prank(vault.safe());
        vault.setSyncMode(SyncMode.None);

        assertTrue(vault.isTotalAssetsValid());
        uint256 reqId = requestRedeem(half, user1.addr);
        assertEq(vault.pendingRedeemRequest(reqId, user1.addr), half);

        vm.prank(vault.safe());
        vault.expireTotalAssets();
        assertFalse(vault.isTotalAssetsValid());

        uint256 reqId2 = requestRedeem(half, user1.addr);
        assertEq(reqId2, reqId, "same epoch expected");
        assertEq(vault.pendingRedeemRequest(reqId, user1.addr), 2 * half);
    }

    function test_requestRedeem_succeedsWhenSyncModeIsSyncDeposit() public {
        test_syncDeposit_simple();
        uint256 half = vault.balanceOf(user1.addr) / 2;
