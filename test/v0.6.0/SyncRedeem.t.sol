...
contract TestSyncRedeem is BaseTest {
    ...
    function test_syncRedeem_haircutShares_burned() public {
        ...
        uint256 haircutAssets = vault.convertToAssetsWithRounding(haircutShares, Math.Rounding.Ceil);
        ...
        // Total assets should decrease by the assets withdrawn + haircut value
        assertEq(vault.totalAssets(), totalAssetsBefore - assets - haircutAssets, "totalAssets not decreased correctly");
    }
    ...
    function test_syncRedeem_pps_stable_when_haircut_non_zero() public {
        // Set up vault with haircut rate but no exit fee
        setUpVault(0, 0, 0, 0, 0);
        ...
        // When haircut > 0, PPS should remain stable because haircut value
        // is also removed from totalAssets (stays in safe, reintegrated at next NAV + settle)
        assertApproxEqAbs(ppsAfter, ppsBefore, 1, "price per share should remain stable when haircut > 0");
        ...
        updateAndSettle(totalAssetsBefore - assets);
        uint256 totalAssetsAfterSettle = vault.totalAssets();

        // assuming that the vault did not produce yield, the total assets should be the same as the total assets before
        // the redeem
        assertEq(totalAssetsAfterSettle - totalAssetsAfterRedeem, haircutAssets, "totalAssets not evolved properly");
    }
    ...
    function test_requestDeposit_succeedsWhenSyncModeIsSyncRedeem() public {
        vm.prank(vault.safe());
        vault.setSyncMode(SyncMode.SyncRedeem);

        uint256 half = assetBalance(user1.addr) / 2;

        assertTrue(vault.isTotalAssetsValid());
        uint256 reqId = requestDeposit(half, user1.addr);
        assertEq(vault.pendingDepositRequest(reqId, user1.addr), half);

        vm.prank(vault.safe());
        vault.expireTotalAssets();
        assertFalse(vault.isTotalAssetsValid());

        uint256 reqId2 = requestDeposit(half, user1.addr);
        assertEq(reqId2, reqId, "same epoch expected");
        assertEq(vault.pendingDepositRequest(reqId, user1.addr), 2 * half);
    }
}
