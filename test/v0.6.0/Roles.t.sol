...
contract TestRoles is BaseTest {
    ...
    function test_lockUpdateSafe() public {
        vm.prank(vault.owner());
        vault.lockUpdateSafe();

        vm.prank(vault.owner());
        vm.expectRevert(SafeUpdateLocked.selector);
        vault.updateSafe(address(0x42));
    }

    function test_lockUpdateSafe_notOwner() public {
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, address(this)));
        vault.lockUpdateSafe();
    }

    function test_lockUpdateSafe_emitsEvent() public {
        vm.expectEmit(true, true, true, true);
        emit SafeLocked(vault.safe());

        vm.prank(vault.owner());
        vault.lockUpdateSafe();
    }

    function test_lockSuperOperator() public {
        vm.prank(vault.owner());
        vault.lockSuperOperator();

        vm.prank(vault.owner());
        vm.expectRevert(SuperOperatorUpdateLocked.selector);
        vault.updateSuperOperator(address(0x42));
    }

    function test_lockSuperOperator_notOwner() public {
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, address(this)));
        vault.lockSuperOperator();
    }

    function test_lockSuperOperator_emitsEvent() public {
        vm.expectEmit(true, true, true, true);
        emit SuperOperatorLocked(superOperator.addr);
