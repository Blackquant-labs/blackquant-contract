...
import {OptinProxyFactoryV3 as OptionProxyFactory_protocolV3} from "@src/protocol-v3/OptinProxyFactory.sol";
import {AccessMode} from "@src/v0.6.0/primitives/Enums.sol";
import {InitStruct} from "@src/v0.6.0/vault/Vault-v0.6.0.sol";

contract SetUp is Test {
    // ERC20 tokens
    // `underlying` defaults to the ASSET env var but is reassignable so tests
    // that need a different decimals regime can swap in a mock via
    // `_useMockUnderlying(decimals)` before calling `setUpVault(...)`.
    ERC20 underlying = ERC20(vm.envAddress("ASSET"));
    ...
    /// @notice Deploy a mock ERC20 and use it as the vault's underlying for
    /// the rest of the test. MUST be called before `setUpVault(...)`.
    /// Flips `underlyingIsNativeToken` to `false` so native-eth branches are
    /// disabled in tests that opt into this override.
    function _useMockUnderlying(
        uint8 decimals_
    ) internal {
        underlying = ERC20(address(new MockERC20("MockUnderlying", "MUNDER", decimals_)));
        underlyingIsNativeToken = false;
    }
    ...
    function setUpVault(
        ...
        if (proxy) {
            bytes memory call_data = abi.encodeWithSignature(
                ...
                abi.encode(initStruct),
                factory.registry(),
                factory.wrappedNativeToken()
            );
